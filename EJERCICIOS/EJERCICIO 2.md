...
- [ ] Entiendo cuándo usar Implicit Wait
- [ ] Entiendo cuándo usar Explicit Wait
- [ ] Sé implementar Explicit Wait sin ExpectedConditions
- [ ] Sé qué excepciones se lanzan
- [ ] Completé las 3 tareas prácticas
- [ ] Respondí correctamente el análisis de código

---

## 🚀 Bonus: Casos Reales

### Caso 1: Login con campo dinámico
```php
// Email siempre existe
$email = $driver->findElement(WebDriverBy::id('email'));
$email->sendKeys('user@example.com');

// Tras escribir email, aparece campo de contraseña
$wait = new WebDriverWait($driver, 5);
$password = $wait->until(function ($driver) {
    try {
        $elem = $driver->findElement(WebDriverBy::id('password'));
...
### Caso 2: Modal que aparece
```php
// Haz clic en botón
$btn = $driver->findElement(WebDriverBy::id('open-modal'));
$btn->click();

// Espera el modal
$wait = new WebDriverWait($driver, 5);
$modal = $wait->until(function ($driver) {
    try {
        $m = $driver->findElement(WebDriverBy::className('modal'));
        return ($m->isDisplayed() && !empty($m->getText())) ? $m : null;
    } catch (Exception $e) {
        return null;
    }
});

echo "Modal content: " . $modal->getText();
```

### Caso 3: Elemento que cambia de estado
```php
// Espera a que un loader desaparezca
$wait = new WebDriverWait($driver, 10);
$wait->until(function ($driver) {
    try {
        $loader = $driver->findElement(WebDriverBy::id('loader'));
        return !$loader->isDisplayed() ? true : null; // Continúa cuando NO está visible
    } catch (Exception $e) {
        return true; // Si no existe, está listo
    }
});

echo "Loader desapareció, página cargada\n";
```

---

## 📊 Resumen: Implicit vs Explicit

| | Implicit | Explicit |
|---|----------|----------|
| **Se configura** | 1 vez al iniciar | Cada vez que se necesita |
| **Timeout** | Global (10s) | Local (5s, 10s, etc.) |
| **Se aplica a** | Todos los findElement() | Solo el wait especificado |
| **Cuándo usar** | Elementos que siempre existen | Elementos dinámicos |
| **Sin ExpectedConditions** | N/A | Usa callable/lambda |
| **Ejemplo** | Campo de login | Campo que aparece tras escribir email |
